...
contract Exchange is ERC20 {
    ...
    // getReserve returns the balance of 'token' held by 'thus' account
    ...
    function addLiquidity(uint256 amountOfToken) public payable returns (uint256) {
        ...
        // if reserve is empty is empty, take any user supplied value for initial liquidity
        ...
        require(amountOfToken >= mintTokenAmountRequired, "Insufficient amount of tokens provided");
    ...
    function ethToTokenSwap(uint256 mintTokensToReceive) public payable {
        uint256 tokenReserveBalance = getReserve();
        uint256 tokensToReceive =
            getOutputAmountFromSwap(msg.value, address(this).balance - msg.value, tokenReserveBalance);

        require(tokensToReceive >= mintTokensToReceive, "Tokens received are less than minimum tokens expected");
